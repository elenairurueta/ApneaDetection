...
class Tester_SaO2:
    """Class to test the model with testset"""
    def __init__(self, model:DualInputModel, testset:Subset, batch_size:int=32, device = "cpu", best_final = ""):
        """
        Initializes the Tester object.
    ...
    def test(self):
        ...
        #Iterate over each batch in the test_loader:
        for X_EEG_test, X_SaO2_test, y_test in self.__test_loader__:
            X_EEG_test, X_SaO2_test, y_test = X_EEG_test.to(self.device), X_SaO2_test.to(self.device), y_test.to(self.device)
            #Get the model's predictions for the batch:
            y_test_pred = self.__model__(X_EEG_test, X_SaO2_test)
            y_test_pred = y_test_pred.round()
            self.__all_preds__.extend(y_test_pred.detach().cpu().numpy().tolist())
            self.__all_labels__.extend(y_test.detach().cpu().numpy().tolist())
            # Save instances where the predictions were incorrect:
            self.__save_wrong_predictions__(X_EEG_test.cpu(), X_SaO2_test.cpu(), y_test.cpu(), y_test_pred.cpu())

    def __save_wrong_predictions__(self, X_EEG_test:Tensor, X_SaO2_test:Tensor, y_test:Tensor, y_test_pred:Tensor):
        """
        Saves instances where the model's predictions were incorrect.
        ...
        # Iterate over each instance in the test data:
        for idx in range(0,len(X_EEG_test)):
            #If the prediction does not match the true label, append the instance to the list of wrong predictions
            if y_test[idx][0] != y_test_pred[idx][0]:
                self.__wrong_predictions__.append([X_EEG_test[0], X_SaO2_test[0], int(y_test_pred[idx][0]), int(y_test[idx][0])])
