...
metrics_acum_total = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], 'AUC': []}
cm_acum_total = []
metrics_std_total = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], 'AUC': []}
cm_std_total = []
best_metrics_acum_total = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], 'AUC': []}
best_cm_acum_total = []
best_metrics_std_total = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], 'AUC': []}
best_cm_std_total = []

txt_file = ""
name0 = f'model_DosSenales_C3O1_SaO2'
if not os.path.exists(models_path + '/' + name0):
    os.makedirs(models_path + '/' + name0)
        
for fold in range(0,10):        

    metrics_acum = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], 'AUC': []}
    cm_acum = []
    best_metrics_acum = {'Accuracy': [], 'Precision': [], 'Sensitivity': [], 'Specificity': [], 'F1': [], 'MCC': [], "AUC": []}
    best_cm_acum = []
    ...
    for file in files: 
        ...
        ds = ApneaDataset_SaO2.load_dataset(f"./data/ApneaDetection_HomePAPSignals/datasets/dataset_file_1600{file:03d}_EEG_C3O1_SaO2.pth")
    ...
    for i in range(0,5):
        ...
        trainer = Trainer_SaO2(
            ...
            device = device)
        trainer.train(models_path + '/' + name0 + '/' + name1, verbose = False, plot = False, save_best_model = False) # + '/' + name2
    ...
    metric_text = (f"Accuracy: ({metrics_mean['Accuracy']*100:.2f}±{metrics_std['Accuracy']*100:.2f})%\n"
                ...
                f"AUC: ({metrics_mean['AUC']:.3f}±{metrics_std['AUC']:.3f})")
    ...
    #             f"AUC: ({best_metrics_mean['AUC']:.3f}±{best_metrics_std['AUC']:.3f})")
...
metric_text = (f"Accuracy: ({mean_metrics['Accuracy']*100:.2f}±{std_metrics['Accuracy']*100:.2f})%\n"
            ...
            f"AUC: ({mean_metrics['AUC']:.3f}±{std_metrics['AUC']:.3f})")
plt.gcf().text(0.1, 0.1, metric_text, ha='center', fontsize=10, bbox=dict(facecolor='white', alpha=0.8, edgecolor='gray'))
