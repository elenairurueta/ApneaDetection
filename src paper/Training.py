...
class Trainer:
    ...
    def train_one_epoch(self):
        ...
        for X_batch, y_batch in self.train_loader:
            X_batch, y_batch = X_batch.to(self.device), y_batch.to(self.device).long()  # Convertir y_batch a Long
        ...
        y_pred = torch.cat(all_y_pred).detach().cpu()
        y_pred = torch.argmax(y_pred, dim=1).numpy().tolist()  # Convierte las probabilidades en clases discretas
    ...
    def plot_accuracies_losses(self, models_path, plot:bool):
        ...
        if os.path.exists(models_path):
            PATH = os.path.join(models_path, 'acc_loss.png')
            plt.savefig(PATH)
    ...
    def write_txt(self, models_path):
        ...
        """           
        if os.path.exists(models_path):
            PATH = models_path + f'/training.txt'
            f = open(PATH, "w")
            f.write(self.text)
            f.close()